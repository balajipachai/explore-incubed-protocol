...
contract NodeRegistryLogic {
    ...
    uint256 public timestampAdminKeyActive;
    ...
    uint256 internal constant YEAR_DEFINITION = 1 days * 365;

    /// limit for ether per node in the 1st year
    uint256 public maxDepositFirstYear;

    /// min deposit required for registering a node
    uint256 public minDeposit;

    /// version: major minor fork(000) date(yyyy/mm/dd)
    uint256 public constant VERSION = 12300020190709;
    ...
    constructor(
        BlockhashRegistry _blockRegistry,
        NodeRegistryData _nodeRegistryData,
        uint256 _minDeposit
    ) public {
        require(
            address(_blockRegistry) != address(0x0),
        ...
        adminKey = msg.sender;
        require(
            address(_nodeRegistryData) != address(0x0),
    ...
    function adminRemoveNodeFromRegistry(address _signer) external onlyAdmin {
        // solium-disable-next-line security/no-block-members
        require(block.timestamp <= timestampAdminKeyActive, "only in 1st year"); // solhint-disable-line not-rely-on-time

        NodeRegistryData.SignerInformation memory si = nodeRegistryData
            .getSignerInformation(_signer);
        require(si.stage == uint256(Stages.Active), "wrong stage");
        NodeRegistryData.In3Node memory in3Node = nodeRegistryData
            .getIn3NodeInformation(si.index);

        nodeRegistryData.unregisteringNode(_signer);
        nodeRegistryData.adminSetStage(
            _signer,
            uint256(Stages.DepositNotWithdrawn)
        );
    ...
    function registerNodeFor(
        ...
        bytes32 tempHash = keccak256(
            abi.encodePacked(_url, _props, _weight, msg.sender)
        );
    ...
    function returnDeposit(address _signer) external {
        ...
        require(si.stage == uint256(Stages.DepositNotWithdrawn), "wrong stage");
        // solium-disable-next-line security/no-block-members
        require(si.lockedTime <= block.timestamp, "deposit still locked"); // solhint-disable-line not-rely-on-time

        uint256 depositAmount = si.depositAmount;
    ...
    function revealConvict(
        ...
        require(
            evmBlockhash != _blockhash,
            "you try to convict with a correct hash"
        );

        bytes32 wrongBlockHashIdent = keccak256(
            abi.encodePacked(_blockhash, msg.sender, _v, _r, _s)
        );

        uint256 convictBlockNumber = nodeRegistryData.convictMapping(
            msg.sender,
            wrongBlockHashIdent
        );
        ...
        require(
            block.number > convictBlockNumber + 2,
            "revealConvict still locked"
        );
        require(
            ecrecover(
                keccak256(
        ...
        NodeRegistryData.SignerInformation memory si = nodeRegistryData
            .getSignerInformation(_signer);
        require(
            si.stage == uint256(Stages.Active) ||
                si.stage == uint256(Stages.DepositNotWithdrawn),
            "wrong stage"
        );

        uint256 deposit = 0;
        if (si.stage == uint256(Stages.Active)) {
            NodeRegistryData.In3Node memory in3Node = nodeRegistryData
                .getNodeInfromationBySigner(_signer);
            ...
            nodeRegistryData.adminSetStage(_signer, uint256(Stages.Convicted));
            emit LogNodeRemoved(in3Node.url, _signer);
        } else {
            deposit = si.depositAmount;

            si.stage = uint256(Stages.Convicted);
            si.depositAmount = 0;
            nodeRegistryData.adminSetSignerInfo(_signer, si);
        }
    ...
    function unregisteringNode(address _signer) external {
        NodeRegistryData.SignerInformation memory si = nodeRegistryData
            .getSignerInformation(_signer);
        require(si.stage == uint256(Stages.Active), "wrong stage");
        require(si.owner == msg.sender, "not the owner");
    ...
    function updateNode(
        ...
        NodeRegistryData.In3Node memory node = nodeRegistryData
            .getNodeInfromationBySigner(_signer);
        ...
        nodeRegistryData.updateNode(_signer, _url, _props, _weight, deposit);

        emit LogNodeUpdated(node.url, _props, _signer, deposit);
    }
    ...
    /// @param _signer IN3 Node Signer details
        ...
        )
    {
        NodeRegistryData.SignerInformation memory si = nodeRegistryData
            .getSignerInformation(_signer);
        return (si.lockedTime, si.owner, si.stage, si.depositAmount, si.index);
    }

    /// @notice Get IN3 Node Information
    /// @param _index the index at which the contract is stored
    function getIn3NodeInfoByIndex(uint256 _index)
        external
        view
        returns (
            string memory _url,
    ...
    {
        NodeRegistryData.In3Node memory in3Node = nodeRegistryData
            .getIn3NodeInformation(_index);
        return (
            in3Node.url,
            in3Node.deposit,
            in3Node.registerTime,
            in3Node.props,
            in3Node.weight,
            in3Node.signer,
            in3Node.proofHash
    ...
    /// @notice Get IN3 Node Information
    /// @param _signer the node signer
    function getIn3NodeInfoBySigner(address __signer)
        external
        view
        returns (
            string memory _url,
    ...
    {
        NodeRegistryData.In3Node memory in3Node = nodeRegistryData
            .getNodeInfromationBySigner(__signer);
        return (
            in3Node.url,
            in3Node.deposit,
            in3Node.registerTime,
    ...
    function _checkNodePropertiesInternal(uint256 _deposit) internal view {
        ...
        // solium-disable-next-line security/no-block-members
        if (block.timestamp < timestampAdminKeyActive) {
    ...
    function _registerNodeInternal(
        ...
        require(si.stage == uint256(Stages.NotInUse), "signer already in use");
