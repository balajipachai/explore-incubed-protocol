...
contract NodeRegistryData {
    ...
    event LogDepositReturned(address nodeOwner, uint256 amount);

    struct In3Node {
        string url; /// the url of the node
        uint256 deposit; /// stored deposit
    ...
    struct SignerInformation {
        ...
        uint256 stage; /// state of the address
        uint256 depositAmount; /// amount of deposit to be locked, used only after a node had been removed
        uint256 index; /// current index-position of the node in the node-array
    }
    ...
    mapping(address => mapping(bytes32 => uint256)) public convictMapping;
    ...
    function adminSetLogic(address _newLogic)
        external
        onlyLogicContract
        returns (bool)
    {
    ...
    /// @return true if successfull
    function adminSetNodeDeposit(address _signer, uint256 _newDeposit)
        external
        onlyLogicContract
        returns (bool)
    {
        ...
        require(
            node.signer == _signer,
            "not the correct signer of the in3-node"
        );
    ...
    /// @return true if successfull
    function adminSetStage(address _signer, uint256 _stage)
        external
        onlyLogicContract
        returns (bool)
    {
    ...
    /// @return true if successfull
    function adminSetSupportedToken(IERC20 _newToken)
        external
        onlyLogicContract
        returns (bool)
    {
    ...
    /// @return true if successfull
    function adminSetTimeout(uint256 _newTimeout)
        external
        onlyLogicContract
        returns (bool)
    {
    ...
    function adminTransferDeposit(address _to, uint256 _amount)
        external
        onlyLogicContract
        returns (bool)
    {
    ...
    function setConvict(bytes32 _hash, address _caller)
        external
        onlyLogicContract
    {
    ...
    function updateNode(
        ...
        emit LogNodeUpdated(node.url, _props, _signer, node.deposit);
    ...
    function getIn3NodeInformation(uint256 _index)
        external
        view
        returns (In3Node memory)
    {
        return nodes[_index];
    }
    ...
    function getSignerInformation(address _signer)
        external
        view
        returns (SignerInformation memory)
    {
    ...
    function getNodeInfromationBySigner(address _signer)
        external
        view
        returns (In3Node memory)
    {
    ...
    /// @return true when successfull
    function adminSetSignerInfo(address _signer, SignerInformation memory _si)
        public
        onlyLogicContract
        returns (bool)
    {
    ...
    function _calcProofHashInternal(In3Node memory _node)
        internal
        pure
        returns (bytes32)
    {
    ...
    function _unregisterNodeInternal(
        SignerInformation storage _si,
        In3Node memory _n
    ) internal {
        // solium-disable-next-line security/no-block-members
    ...
    function _removeNodeInternal(uint256 _nodeIndex) internal {
        ...
        signerIndex[nodes[_nodeIndex].signer]
